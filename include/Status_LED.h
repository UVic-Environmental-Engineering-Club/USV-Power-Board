...
#define LED_RMT_TX_GPIO			27      //GPIO pin number where the WS2812 strip is connected
#define T0H                     14      //0 bit high time. Time interval the signal should be high for when a 0 bit is transmitted
#define T1H                     52      //1 bit high time. Time interval the signal should be high for when a 1 bit is transmitted
#define TL                      52      //low time for either bit. Time interval the signal should be low for followed by the high time for either bit


//colours are saved as GRB, with 2 hex digits per colour (ie 0xGGRRBB). Colours are very bright so I played around with them a bit to get them looking alright.
